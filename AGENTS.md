...
- Before debugging recurring errors, check `ERROR_PATTERNS.md` for recorded symptoms/fix patterns first
- When implementing fixes, never hardcode magic strings when logic or authoritative data checks can be used instead
- **Text > Brain** 📝
...
From now on, any chats from future sessions that lead to actions (once confirmed with you or executed/completed) will be added, updated, or removed from the `TO-DO.md` file in the workspace. This ensures the to-do list always reflects the latest state of tasks.
